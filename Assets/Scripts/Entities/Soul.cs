...
public class Soul : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            transform.position = Vector3.Lerp(transform.position, GameObject.Find("Player").transform.position, 0.005f);
        ...
        if (health < 1)
        {
            ...
            GameObject.Find("UIManager").GetComponent<UIManager>().session.soulsKilled++;

            if (GameObject.Find("UIManager").GetComponent<UIManager>().session.soulsKilled >= 5 && GameObject.Find("UIManager").GetComponent<UIManager>().session.achievements[1] == false)
            {
                StartCoroutine(GameObject.Find("UIManager").GetComponent<UIManager>().Achievement(1));
        ...
        {
            GameObject.Find("UIManager").GetComponent<HUD>().UpdateHealth(-5);
